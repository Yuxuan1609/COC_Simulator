        ...
        response = self.monitor.call(
            ...
            system="你是一个优秀的跑团KP，擅长生动、沉浸的叙事。"
                   ...
                   "\n- brief 与 narrative 严格呼应：brief 陈述事实，narrative 展开感受"
                    "\n- scene_update: 当本轮行动导致场景发生「持久可见变化」时，输出变化后的完整场景描述。该字段直接更新玩家可见的场景描述（L1），后续回合使用新描述。变化包括但不限于：物品被移走/拾取、门被打开/关上、出现血迹/痕迹、光源亮起/熄灭、NPC 出现/离开、敌人被击败后留下尸体或痕迹。即兴行为（仅叙述性描写、无实际影响）不填此字段。无变化时填空字符串。"
