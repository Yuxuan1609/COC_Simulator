...
## src/game/messages.py (304 行) — 消息类型 / 契约
...
| `PlayerTurnResult` | `status, brief, narrative, pending_interaction, player_snapshot, skill_results, combat, combat_init, ending, game_over, timestamp, diagnostics, debug` | **run_turn 玩家面契约返回**；`debug` 可选（默认 None；debug ON 为 `{evaluated, matched}`） | 290 |

---

## src/game/agents/keeper.py (986 行) — Keeper 回合编配
...
| `_detect_direct_drop` | `(raw) -> tuple[str, str] \| None` | F17：丢/扔/放下/丢掉 + 最长持有名（item_manager / weapons）；否定词不触发；点名未持有返回 `("missing", name)`；无名不触发 | 539 |
| `_drop_to_scene` | `(kind, ref) -> str` | F17：从背包/武器槽移除一件（武器 `remove_weapon` 只卸一把）；仅合并 **exposed** 同 kind+ref（物品/武器 quantity+=1），仅有 hidden 则 append 新暴露行，永不改 hidden；再 `_sync_scene_weapons_from_items` | 563 |
| `_devour_standoff_for_boss` | `(standoff_prompt, combat_init_result, all_outcomes, enrich_input) -> None` | F3：Boss 强制战吞掉对峙——撤回 standoff 播种/话术，avoidable 敌人并入 Boss 战（at 与 event 两条 engage 通路共用）；C 吞对峙① / E event 吞对峙② 共用 | 585 |
| `_build_frozen_response` | `(exc)` | TurnFrozenError → FROZEN TurnResult | 185 |
| `_scan_ending` | `(outcomes, author)` | 检查 ##END_*## 结局标记并触发 | 193 |
| `complete_combat_turn` | `(original_input, combat_result)` | 战斗后回放 enrich→curate；入口先把 outcome 记入编年史（record_combat_end，CLI/前端/auto 全通路覆盖） | 210 |
| `resolve_standoff` | `(standoff_state, player_input)` | 对峙：LLM 匹配技能 → D100（F19：`env_check_modifier`）→ 特质修正；说服族判定经 normalize_skill_name 归一（魅惑/说服两族，旧名话术/恐吓落入说服） | 253 |
| `_check_boss_requirements` | `(boss_entity, player_action)` | Boss 遭遇触发条件检查 | 333 |
| `_evaluate_boss_soft_condition` | `(soft_condition, player_action, boss_name)` | Boss 软条件 LLM 评估 | 358 |
| `_inject_npc_at` | `()` | 当前场景 NPC bound entity → 注入 node（F23：completed 且非 repeatable 才跳过；N1：bound_interactions attitude_min 不满足则跳过，注入 extra 带 npc_name/attitude_min） | 385 |
| `_apply_pending` | `()` | 应用延迟副作用 + 移动 + NPC 跟随实体注入；GrantWeapon(scene="") 直接入包（无 offer）；Restart 时由 runner 在重入 A 前调用（A 入口会 clear pending） | 438 |
| `_parse` | `(raw) -> list[dict]` | LLM parse：玩家输入 → action 列表（live system= 含 `KEEPER_PARSE_MADNESS_RULE`）；debug ON 时对 interaction/event/auto_trigger 的 id 调 `_record_match` | 610 |
| `_enrich` | `(judged_entities, user_input) -> dict` | LLM enrich：合并判定结果 | 649 |
| `_log_agent_response` | `(filename, data)` | 记录 agent 响应日志 | 679 |
| `_find_entity_by_id` | `(entity_id)` | graph+NPC+boss 按 ID 查找（F23：NPC bound completed 且非 repeatable 才跳过；N1：extra 写入 npc_name/attitude_min） | 692 |
| `_process_deterministic_only` | `(turn_input)` | 深度超限/降级时纯确定性执行（A 的 `depth>=MAX` 与 runner Restart 后 `depth>=MAX` 两处入口） | 736 |
| `_build_world_brief` | `()` | 构建 pre-parse 用世界简报 | 757 |
| `_build_world_snapshot` | `()` | 构建世界快照 dict | 773 |
| `_infer_time_category` | `(entity)` | 实体时间类别推断 | 785 |
| `_run_time_agent` | `(action_summaries, raw)` | 调用 TimeAgent 评估时间 | 792 |
| `_build_scene_context_for_author` | `()` | 构建 Author 场景上下文（含 chronicle 渲染） | 799 |
| `_integrate_supplement` | `(structural_edit, author, intent, reasoning)` | 补充管线 → 集成到 graph；成功后 record_patch(level="structural") | 816 |
| `_load_scene_into_graph` | `(scene_name, scene_data)` | 新场景注入 graph（补充管线产物） | 905 |
| `_integrate_patch` | `(patch)` | ModulePatch 实体集成 + record_patch(level="patch")；entity_ids 记集成后真实 id | 959 |
...
### `context.py`（50 行）— 管线契约

| 类 | 字段/说明 | 作用 | 行号 |
|----|-----------|------|------|
| `TurnContext` | `turn_input, author, depth, raw, trace` | 输入侧（只读）；raw 可被 pre_parse 改写；`trace` debug ON 为共享 list，OFF 为 None | 10 |
| `TurnAccumulator` | parse/npc/outcomes/enrich_input/combat/boss_accounting/enrichment/ta_result/brief/result 等 | 产出侧累积（= 原 process_turn 局部变量显式分组） | 20 |
...
## src/game/combat.py (1565 行) — 战斗系统 v2
...
| `run_single_round` | `(combat_init, state, action_id, target_ids, player_extra="") -> dict` | 交互式单回合（前端回合制；轮末 @595 `state.round += 1` 前调 `_tick_temporary_effects`） | 436 |
| `_build_single_round_result` | `(state, combat_init) -> dict` | 单回合结果 dict；被支配跳过（weapon="--"/无法动弹）不渲染「未命中」；san_log 一次性渲染 | 601 |
| `_generate_combat_narrative` | `(state, player, scene, log_dir)` | 战斗叙事生成 | 661 |
| `_init_combat` | `(combat_init) -> CombatState` | 初始化：展开 quantity 群组，按 DEX 排先攻；player_san_max=player.derived.SAN_MAX（F2 @763）；末尾目睹 SAN check @788-815（F9 收口）：expanded_enemies 按 enemy_ref 去重，去重键=局部 seen_refs ∪ world.san_seen_sources（world 非 None 且有该属性时 check 后回写 ref 入全局集入档，跨场跨档同恐怖源只首次目睹 check；world=None 退化为场内去重；同 ref 群组只一次），目睹组=注释不含"攻击"的第一组，**B20 单轨：扣 player.derived.SAN（下限 0）+ state.player_san 镜像 @803-806**+san_log 追加叙事行；F5：loss>0 时经 getattr 防御式调 world.on_san_loss @807-812（触发疯狂 text 追加「（疯狂侵袭）」） | 742 |
| `_match_action` | `(raw_input, available)` | 文本 → 动作 ID 匹配 | 820 |
| `_get_player_actions` | `(player, environment_actions)` | 固定动作列表（拳/踢/回避/逃跑/武器/环境/施法--known_spells∩combat 类生成 cast_<id> 动作） | 852 |
| `_skill_value` | `(player, skill_name)` | 技能值查询 | 905 |
| `_resolve_player_action` | `(state, player, action_id, target_iid, environment_actions)` | 执行玩家动作（cast_* 前缀走 cast_spell 分支：习得/MP 硬门 -> 扣减 -> opposed/常规检定 -> **effect 原子数组遍历 @963-1041**（T9 重写，检定成功才结算）：damage 保留 _roll_damage+ignore_armor+死亡标记 / heal（formula 掷骰或 delta 回退，clamp HP_MAX，**B21：结算后 state.player_hp 镜像 @987**）/ mp_change（clamp 0..MP_MAX）/ markup（parse_markup_all+apply_side_effects 走 self.world，**B20：结算后 state.player_san 镜像 @1000；B21：state.player_hp 镜像 @1004**，无 world 跳过+warning）/ timed（挂 world.player.timed_effects，同 id refresh，expire_at=clock.game_time+minutes，有 interval/payload 则拷入条目，无 world/player 跳过+warning）/ buff（挂 state.temporary_effects，T10 消费）/ control（写 target.controlled_rounds，T11 已消费：_resolve_enemy_action 顶部跳过行动）/ narrative（拼 action.narrative）/ 未知 type `[unknown:{t}]` 降级永不报错）；F5+B20：need_san 扣 derived.SAN 后镜像 state.player_san @936-938 并调 world.on_san_loss @940（施法损失计疯狂） | 916 |
| `_get_tier` | `(roll, skill_value)` | COC 四级检定 | 1165 |
| `_select_enemy_attack` | `(enemy)` | 按权重随机选攻击 | 1177 |
| `_select_enemy_target` | `(state, enemy)` | 敌人选目标 | 1185 |
| `_resolve_enemy_action` | `(state, enemy, player)` | 执行敌人动作；顶部 control 检查跳过；命中技能优先读 attack.skill_value（>0），否则 (DEX+POW)//2，再加 dodge_bonus（F13）；buff 减伤；**B21 单轨：伤害扣 player.derived.HP + state.player_hp 镜像 @1244-1248**；被攻击 SAN check @1247-1263（F9 收口：同场同 enemy_ref 只首次命中触发，state.san_attacked_refs 场内去重不入档，multi_attack 不叠加）；**B20 单轨：扣 player.derived.SAN + state.player_san 镜像 @1256**；F5：loss>0 时经 getattr 防御式调 world.on_san_loss @1257-1262（触发疯狂 text 追加「（疯狂侵袭）」进 narrative） | 1189 |
| `_tick_temporary_effects` | `(state)` | 轮末递减（T10）：temporary_effects 各条 rounds-1、归零移除（`rounds-1 > 0` 存活过滤）；enemy.controlled_rounds 递减（T11 消费：_resolve_enemy_action 顶部检查）；F10：world.player.timed_effects 中 `interval==round` 条目每轮调 `apply_effect_payload`（单条 try/except 隔离，失败记 `[F10] payload 结算失败` 继续下条；**B21：结算后 state.player_hp=wp.derived.HP 镜像 @1293**）；调用点 run_combat @406 / run_single_round @595 / run_game.py 交互循环 @521（均在 `state.round += 1` 前，共 3 处） | 1274 |
| `_check_phase` / `_apply_phase` | — | Boss 阶段切换 | 1300 / 1324 |
| `_any_special_rules` | `(combat_init, enemies)` | 是否有 special_rules 需要 LLM | 1345 |
| `_build_battle_snapshot` | `(state, player, boss_phase)` | LLM 用战斗快照 | 1355 |
| `_build_round_result` | `(state, player_actions, enemy_actions, round_num)` | RoundResult 构建 | 1374 |
...
| `Judge._trace_eval` / `_trace_match` | `(entity_id, ...)` | turn_trace 只读 append；`_turn_trace is None` 则立即返回（零负载） | 45 / 60 |
| `Judge.check_auto_triggers` | `()` | 触发当前场景满足简单条件且 time_condition 匹配的全部 AT；list 先 json.dumps 再 check_time_condition(tc, day, tod)，空 []/"[]" 放行；time 不满足时 `_trace_eval(gate=time)` | 75 |
...
## src/scenario_core.py (2144 行) — 数据模型 + 世界状态
...
| 方法 | 签名 | 作用 | 行号 |
|------|------|------|------|
| `record_turn` | `(turn_number, raw_input, result, world)` | 每回合末记录事件（窗口15）+ entity_results（截断100）；通道：intent/entities/at/spawn(SpawnEnemy 副作用)/pending/combat start/ending/npc/boss diff。在 narrate **之前** 调用，不含叙事全文 | 1924 |
| `record_narrative` | `(turn_number, brief, narrative)` | F39：narrate 成功后写入 `narrative_log`（窗口200，brief/narrative 截 2000 字）。不进 `render_for_author` | 1965 |
| `_diff_boss` | `(world) -> list[str]` | Boss 增量 diff（engage/defeated），基准集 `_boss_seen_spawned/_boss_seen_dead` 入档防读档重报；逻辑同 llm_player._collect_mech_line | 1973 |
| `record_combat_end` | `(outcome, world)` | 战斗结算后标注当回合 combat_end + 同回合补 boss defeated（由 keeper.complete_combat_turn 统一调用） | 1995 |
| `record_patch` | `(turn, level, entity_ids, new_scenes, justification)` | 补丁清单（append-only，justification 截断100）；entity_ids 为集成后真实 id（含 NEW_xxx 回退） | 2006 |
| `compress_events` | `(llm_call)` | LLM 蒸馏预留接口，本期不接线（NotImplementedError） | 2016 |
| `render_for_author` | `(world) -> str` | 渲染【世界真值】（玩家行含 HP/SAN/MP_MAX、武器+关键物品+已知法术、timed_effects 生效中块（描述+剩X分钟，空则不渲染，LLM 可见性 2026-08-21 spec §2.3）、敌人、Boss 块：已开战状态/阶段 + 未遭遇清单）+【已注入内容】+【编年史】**不含** narrative_log | 2022 |
| `_render_event` | `(e) -> str` | 单条事件紧凑渲染（含 combat=end(outcome)） | 2103 |
| `to_dict` / `from_dict` | — | 序列化（events 转 list + boss_seen 两集合 + `narrative_log`；旧档缺键 → 空 deque） | 2123 / 2135 |
...
| `build_snapshot` | 玩家状态快照（统一资源层增 mp_max / known_spells 字段；F5：四文本键 temporary/indefinite/phobia/mania 非空时携 `insanity` 段入 prompt——ledger 键 san_lost_today 等不入，@326-330） | 309 |
| `_recalc_derived` | 重算衍生属性：只重算上限/DB/BUILD/DODGE，当前值（HP/MP/SAN）经 _carry_current 携带或 clamp，SAN 永不重置 | 335 |
| `modify_stat` | `(stat_name, delta)` 支持骰子公式；SIZ->CON 映射（spec 7.2 旧模组兼容）；CON 变化按 HP_MAX=max(1,CON//3) 重算并压 HP | 346 |
| `modify_skill` / `has_item` / `list_items` | - | 430 / 437 / 441 |
...
## src/prompts.py (1190 行) — Prompt 构建（所有 build_* 只构建不调用）
...
| `build_combat_entry_prompt` | 战斗入口判定 | 972 |
| `build_standoff_match_prompt` | 对峙技能匹配 | 997 |
| `build_combat_narrative_prompt` | 战斗叙事 | 1020 |
| `build_stat_narrative_prompt` | 属性变化 → 个人描述增量更新 | 1046 |
| `build_material_fuzzy_prompt` | `(target, catalog_text, quantity=1)` **统一资源层**素材模糊匹配（物品/法术通用，输出 {matched, material, reason}） | 1065 |
| `build_consume_item_fuzzy_prompt` | 旧消耗品模糊匹配兼容包装（scenario_core 通路，读 material 或 item_name 双键） | 1084 |
| `build_time_pressure_assess_prompt` | 时间压力介入判定 | 1095 |
| `build_npc_intent_detect_prompt` | 是否在和 NPC 对话 | 1137 |
| `build_npc_parse_prompt` | NPC 互动解析 | 1158 |
...
| `scene.js` | 场景卡（`updateSceneCard` 写 `lastSceneSnap` @23）、initGame、sendTurn/sendTurnAction（`runTurnRequest` @591：`state.debug` 时 `fd.append("debug","1")`，成功后 `refreshDebugSnapshot` 不 await）、handleTurnResponse（`applyTurnDebug` @454；优先 `slash.text`，残余 `narrative_html` `escapeHtml`）、renderTurnDynamic/renderSkillChips；`addToHistory` @331 空操作（F39 改走 chronicle）；`toggleAutoWin` @657 / `syncAutoWin` @662（`paintSwitch`） |
...
| `game.js` | 入口：`window.*` 桥（含 `toggleHistory`）；`toggleDebug` @28（调 `toggleDebugPanel` 后按 `lastSceneSnap` 重绘场景卡）；`setDebug` 吃 `?debug=`；`paintDebugUi` @57（`syncDebugUi` + 已开则拉 snapshot）；bootstrap `/api/game/state`；DOMContentLoaded 调 `bindHistoryScroll` |

scene.js ↔ combat.js 循环导入（`enterCombatMode` / `addToHistory` 空操作），仅函数内调用，ESM live binding。scene.js → debug.js 单向（无环）。测试：`tests/test_frontend_js_modules.py` + `tests/js/*.test.mjs`（含 `history.test.mjs`）。
