...
（注：san_loss 每轮情境组目前无触发点消费，静默——ISSUES F10 跟踪。）
...
- **目睹组**（情境注释不含 "攻击"）：开战时对每个 enemy_ref 做 SAN check 一次
  - **全局首次目睹才 check**（`world.san_seen_sources` 入档，跨场跨档同恐怖源不重复；群组多实例展开后同 ref 只查一次）
  - 若所有组都含 "攻击" 注释，退回首组作目睹检定
- **被攻击组**（注释含 "攻击"）：敌方命中时 check；**同场同 enemy_ref 只首次命中触发**（场内去重，不入档；multi_attack 每轮多次命中不再叠加）
- **每轮情境组**（如 "每轮在雾中停留"）：目前无触发点消费，静默（ISSUES F10）
