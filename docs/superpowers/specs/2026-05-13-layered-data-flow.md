...
> **⚠ 2026-05-14 更新**: 本文档的第一～十二节描述的是**旧的一次生成模式**（`parse_module()` → `run_pipeline()`），该模式已被四步渐进式流程替代。请以 `2026-05-14-progressive-parser-design.md` 为当前权威设计文档。第十三～十四节的提案内容已通过 brainstorming 确认并正式实施。当前代码状态见 `NEXT-SESSION.md`。
