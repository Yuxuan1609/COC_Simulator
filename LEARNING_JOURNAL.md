...
## 后端 Session 缓存替代全量状态序列化

- **场景**：`CombatState` 包含 `CombatAction` 对象列表和 `EnemyInstance` 复杂对象，无法简洁地序列化为 JSON 供前后端往返传递
- **方案**：后端维护内存 `_combat_sessions: dict[str, CombatState]`，前端每轮只发送 `session_id + action_id + target_ids`，后端直接操作内存对象
- **收益**：(a) 避免 CombatAction/EnemyInstance 的复杂序列化；(b) 状态变更原子性（单进程内直接修改，无并发问题）；(c) 天然支持 Python 对象引用（如 combat_init.player 指针）
- **代价**：页面刷新丢失 session（本版本接受），多进程部署需外部存储（Redis）
- **泛化**：任何"前端交互驱动 + 后端状态机"的场景——回合制战斗、棋盘游戏、卡牌游戏。前端负责 UI 状态，后端负责业务状态，session_id 是桥梁。

## 增量式交互分配模式

- **场景**：`multi_attack=2` 时需要为 2 次攻击分配目标，且允许同一目标被多次攻击。传统多选下拉框无法表达"同一目标×2"
