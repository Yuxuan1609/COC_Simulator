...
> **[P0] 封包发布准备中** — 项目即将封包发布，以下事项需最高优先级推进：
> 1. **文档准备** — 用户手册（安装/启动/游戏指南）、开发者文档（架构/API/扩展）整理完善
> 2. **依赖解析** — 梳理 `requirements.txt` 或 `pyproject.toml`，明确最小依赖集，验证各平台兼容性
> 3. **打包工具链** — Nuitka（生产）/ PyInstaller（fallback）环境搭建、构建脚本、CI 集成
> 4. **分发准备** — 图标资源、版本号规范、更新机制（若有）、免 Python 环境验证
