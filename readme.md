...
| # | 事项 | 说明 | 优先级 |
|----|------|------|:---:|
| U2 | **世界状态系统** | Logger 驱动的状态解读（需考虑法术系统和其他潜在系统依赖） | 高 |
| U9 | **技能系统重修** | 技术难度低但设计难，COC7 规则技能体系本身抽象，不必照搬 | 高 |
| U3 | LLM Provider 抽象 | 支持 OpenAI/Anthropic 多 provider（等地其他改完再做） | 中 |
| U4 | 跨模组持久化 | 调查员永久化、战役系统（低难度，等改完再做） | 中 |
| U6 | 法术体系 | SpellJudge + @grant_spell（低难度，等改完再做） | 中 |
| U7 | LLM 调用成本优化 | 各步骤最低配置（低难度，等改完再做） | 中 |
